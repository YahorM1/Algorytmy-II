...
std::pair<std::vector<int>, std::vector<int>> brute_force(std::vector<std::vector<float>>& cost_matrix, bool maximize = false) {

    int h = cost_matrix.size();
    int w = cost_matrix[0].size();
    //std::cout << "h = " << h << std::endl;
    //std::cout << "w = " << w << std::endl;

    if (maximize) {
        std::vector<std::vector<float>> negated_cost_matrix = cost_matrix;
